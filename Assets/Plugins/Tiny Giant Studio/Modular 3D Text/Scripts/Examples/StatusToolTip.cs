...
namespace TinyGiantStudio.Text.Example
{
    public class StatusToolTip : MonoBehaviour
    {
        ...
        [Tooltip("This uses itself as pool holder, not pool manager like the text and the text prefabs MUST have unique names because that's what is used for pool key")]
        public bool pooling = true;

        readonly Dictionary<string, Queue<GameObject>> _poolDictionary = new Dictionary<string, Queue<GameObject>>();
        ...
        GameObject GetIObj(int style)
        {
            if (pooling)
            {
                ...
                if (_poolDictionary.ContainsKey(key))
                {
                    ...
                    {
                        GameObject poolItem = _poolDictionary[key].Dequeue();
                        return poolItem;
                    }
                }
            }
