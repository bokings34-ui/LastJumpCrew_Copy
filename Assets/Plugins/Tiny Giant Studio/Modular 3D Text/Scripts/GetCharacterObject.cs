    ...
    public class GetCharacterObject
    {
        ...
        private static MeshLayout ProcessNormalCharacter(char c, Modular3DText text, MeshPostProcess meshPostProcess)
        {
            ...
            else
            {
                ...
                Mesh mesh = creator.GetMesh(text.Font.GetEntityId(), text.Font.TypeFace, text.Font.sizeXYInput, text.Font.sizeZInput, text.Font.autoSmoothAngleInput, text.Font.averageYValue, c);
                if (!mesh || text.Font.TypeFace.glyphs[c].GlyphIndex == 0)
                {
        ...
        private static Mesh GetMeshFromFallbackFonts(char c, Modular3DText text, CharacterGenerator creator, Mesh mesh)
        {
            ...
            {
                if (!text.Font.fallbackFonts[i]) continue;

                Mesh tryMesh = creator.GetMesh(
                    text.Font.fallbackFonts[i].GetEntityId(),
        ...
        private static MeshLayout ProcessNormalCharacter(char previousChar, char currentChar, Modular3DText text, MeshPostProcess meshPostProcess)
        {
            ...
            else
            {
                ...
                var mesh = creator.GetMesh(text.Font.GetEntityId(), text.Font.TypeFace, text.Font.sizeXYInput, text.Font.sizeZInput, text.Font.autoSmoothAngleInput, text.Font.averageYValue, currentChar);
                if (!mesh || text.Font.TypeFace.glyphs[currentChar].GlyphIndex == 0)
                {
        ...
        private static GameObject ProcessNormalCharacter(char previousChar, char currentChar, Modular3DText text, GameObject obj, LayoutElement layoutElement, MeshPostProcess meshPostProcess)
        {
            ...
            {
                CharacterGenerator creator = new CharacterGenerator();
                Mesh mesh = creator.GetMesh(text.Font.GetEntityId(), text.Font.TypeFace, text.Font.sizeXYInput, text.Font.sizeZInput, text.Font.autoSmoothAngleInput, text.Font.averageYValue, currentChar);
                if (!mesh || text.Font.TypeFace.glyphs[currentChar].GlyphIndex == 0)
                {
                    mesh = GetMeshFromFallbackFonts(currentChar, text, creator, mesh);
                }
