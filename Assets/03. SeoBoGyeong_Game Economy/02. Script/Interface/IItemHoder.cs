...
namespace LastJumpCrew.SeoBoGyeong.Interaction
{
    ...
    public interface IItemHolder
    {
        ...
        UtilityItemData CurrentItemPrefabData { get; }

        // 기존 아이템을 내려놓고 새 아이템을 들 수 있는지 검사한다.
        bool CanReplaceHeldItem(UtilityItemData itemPrefabData);

        // 새 아이템을 손에 생성한다. interactionSource는 로그/드롭 기준 확장용으로 넘긴다.
        void ReplaceHeldItem(UtilityItemData itemPrefabData, Transform interactionSource);

        // 현재 든 아이템을 드롭 프리팹으로 월드에 배치한다.
        void PlaceHeldItem();

        // 특정 itemId를 가진 아이템을 손에서 제거한다. 배터리 삽입/툴박스 보관에서 사용한다.
        bool TryConsumeHeldItem(int itemId);
    }
}
