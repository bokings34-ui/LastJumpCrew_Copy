...
- Enemy_AT_Device.prefab, Enemy_AT_Player.prefab (실제 스폰 대상, 애니메이션 포함)
- EnemySpawnDataSO (밸런스 데이터: playerAttackEnemyPrefab, deviceAttackEnemyPrefab, enemyCount, spawnInterval)
...
- `targetRoom`: `IRoom` 구현체
- `onFinishedCallback`: `Action<EventBase, bool>` — 성공 / 실패 결과 통보
...
- 사망 시 Die 애니메이션 재생 후(약 1초) 풀 반환
- 전원 사망 시 자동으로 성공 종료

## 주의사항 (중요)
- Enemy는 실제로 Player/장치를 추적하고 공격하며, 공격 성공 시 `IDamageable.ApplyDamage()`를 직접 호출하여 개체 단위 피해를 확정합니다. (이 부분은 이 Bundle이 담당)
- 다만 이 피해가 함선 전체 상태(Ship HP, Module 파괴, 게임 승패 조건 등)에 어떻게 집계·반영되는지는 이 Bundle의 범위 밖입니다.
- 스폰 그룹(`EnemySpawnSetting`)은 씬에 배치 후 인스펙터에서 직접 등록해야 합니다.
...
## 테스트 완료 사항
- `EventManager.Instance.SpawnEvent(EventId.EnemySpawn, room, callback)` 호출로 3마리 순차 스폰 → 추적/공격 동작 → 사망 애니메이션 → 전원 처치 → 성공 콜백 확인
- Console 에러 0 
