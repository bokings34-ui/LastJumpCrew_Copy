...
- `Physics.OverlapSphere`로 범위 내 Player를 직접 스캔하여 서서히 중심으로 당김 (첫 5초간 흡입력 감소 후 소멸)
- 중심 반경(`innerDamageRadius`) 도달 시 주기적으로 데미지
- **Collider 없음**: 범위 판정을 `OverlapSphere`로 직접 처리하므로 트리거 Collider가 불필요함 (FireEffectInstance와 다른 방식)
