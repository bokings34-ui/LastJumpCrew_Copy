...
namespace LastJumpCrew.ParkHanSol.Editor
{
    public static class PHSNetworkAudioWiringAuthoring
    {
        ...
        [MenuItem("Tools/ParkHanSol/BEAVER/Recover Player Audio Only")]
        public static void AuthorPlayerAudioOnly()
        {
            PHSCuratedAssetSfxAuthoring.Author();
            var clips = LoadRequiredClips();
            ConfigurePlayerPrefab(PlayerPrefabPath, clips, false);
        ...
        private static void ValidatePlayerAudioOrThrow()
        {
            foreach (var runtimePath in PHSCuratedAssetSfxAuthoring.AllRuntimePaths)
            {
                if (AssetDatabase.LoadAssetAtPath<AudioClip>(runtimePath) == null)
                {
                    throw new InvalidOperationException(
                        $"PHS_PLAYER_AUDIO_VALIDATE_FAILED reason=curated_clip_missing path={runtimePath}");
                }
            }
        ...
        private static void ValidatePlayerAudioPrefab(string path)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"PHS_PLAYER_AUDIO_VALIDATE_FAILED reason=prefab_missing path={path}");
            }

            var itemFeedback = RequireSingle<NetworkPlayerItemAudioFeedback>(prefab);
            var movementFeedback = RequireSingle<NetworkPlayerMovementAudioFeedback>(prefab);
            var scanner = RequireSingle<TempPlayerInteractionScanner>(prefab);
            var ownerEmitter = RequireNamedComponentInDescendants<NetworkAudioCueEmitter>(
                prefab.transform,
                "PHS_NetworkItemAudio_2D");
            var worldEmitter = RequireNamedComponentInDescendants<NetworkAudioCueEmitter>(
                prefab.transform,
                "PHS_NetworkItemAudio_3D");

            var scannerSerialized = new SerializedObject(scanner);
            if (!itemFeedback.HasRequiredReferences
                || !movementFeedback.HasRequiredReferences
                || scannerSerialized.FindProperty("interactionCuePlayerSource")
                    ?.objectReferenceValue != ownerEmitter)
            {
                throw new InvalidOperationException(
                    $"PHS_PLAYER_AUDIO_VALIDATE_FAILED reason=reference_invalid path={path}");
            }
            ...
            ValidateEmitterBindings(
                ...
                NetworkAudioCue.PlayerJump,
                NetworkAudioCue.VendingInteraction,
                NetworkAudioCue.InteractionFocus,
                NetworkAudioCue.OptionsSaved);
            ValidateEmitterBindings(
                worldEmitter,
                path,
                NetworkAudioCue.ItemPickup,
        ...
        private static void ValidateEmitterBindings(
            NetworkAudioCueEmitter emitter,
            string prefabPath,
            params NetworkAudioCue[] expectedCues)
        {
            var serialized = new SerializedObject(emitter);
            var source = serialized.FindProperty("audioSource")?.objectReferenceValue;
            var bindings = serialized.FindProperty("cueBindings");
            if (source == null || bindings == null || bindings.arraySize != expectedCues.Length)
            {
                throw new InvalidOperationException(
                    $"PHS_PLAYER_AUDIO_VALIDATE_FAILED reason=emitter_contract_invalid path={prefabPath} emitter={emitter.name}");
            }

            for (var index = 0; index < expectedCues.Length; index++)
            {
                var expectedCue = expectedCues[index];
                var binding = bindings.GetArrayElementAtIndex(index);
                var cue = binding.FindPropertyRelative("cue");
                var clip = binding.FindPropertyRelative("clip");
                var expectedClip = AssetDatabase.LoadAssetAtPath<AudioClip>(
                    PHSCuratedAssetSfxAuthoring.GetCuePath(expectedCue));
                if (cue == null
                    || clip == null
                    || cue.intValue != (int)expectedCue
                    || clip.objectReferenceValue != expectedClip)
                {
                    throw new InvalidOperationException(
                        $"PHS_PLAYER_AUDIO_VALIDATE_FAILED reason=cue_binding_invalid path={prefabPath} emitter={emitter.name} index={index} expected={expectedCue}");
                }
            }
        }
