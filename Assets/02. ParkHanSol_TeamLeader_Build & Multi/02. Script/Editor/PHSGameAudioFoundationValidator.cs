...
namespace LastJumpCrew.ParkHanSol.Editor
{
    public static class PHSGameAudioFoundationValidator
    {
        ...
        public static void Validate()
        {
            ...
            ValidateFirePresentationPrefab(mixer, errors);
            ValidateSettingsPrefabs(mixer, errors);
            ValidateSettingsRuntimeContract(errors);
            ValidateLobbyMasterOwnership(errors);
            if (errors.Count > 0)
            {
            ...
            Debug.Log(
                "PHS_GAME_AUDIO_FOUNDATION_VALIDATION_PASSED clips=23 pcm_adpcm=true preload=true mixer=4groups sources=routed priority=tiered headroom=true fire=loop_3d_sfx settings=2");
        }
        ...
        private static void ValidateFirePresentationPrefab(
            AudioMixer mixer,
            ICollection<string> errors)
        {
            var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(
                FirePresentationPrefabPath);
            var fireLoopClip = AssetDatabase.LoadAssetAtPath<AudioClip>(
                FireLoopClipPath);
            if (prefab == null)
            {
                errors.Add(
                    $"fire_audio_prefab_missing path={FirePresentationPrefabPath}");
                return;
            }

            if (fireLoopClip == null)
            {
                errors.Add($"fire_loop_clip_missing path={FireLoopClipPath}");
                return;
            }
            ...
            {
                errors.Add(
                    $"fire_audio_source_count actual={sources.Length} expected=1");
                return;
            }

            var source = sources[0];
            if (source.clip != fireLoopClip
                || source.outputAudioMixerGroup == null
                || source.outputAudioMixerGroup.audioMixer != mixer
                || source.outputAudioMixerGroup.name != "SFX"
                || source.playOnAwake
                || !source.loop
            ...
            {
                errors.Add(
                    $"fire_audio_contract clip={AssetDatabase.GetAssetPath(source.clip)} group={source.outputAudioMixerGroup?.name ?? "null"} spatial={source.spatialBlend} priority={source.priority}");
            }
        }
            ...
            foreach (var marker in new[]
                     {
                         ...
                         "ResolveMixerHeadroomDecibels(parameterName)",
                         "MasterHeadroomDecibels = -3f",
                         "UiHeadroomDecibels = -2f",
                         "EffectsHeadroomDecibels = -2f",
                         "EnvironmentHeadroomDecibels = -4f",
        ...
        private static void ValidateLobbyMasterOwnership(
            ICollection<string> errors)
        {
            var path = Root +
                "/02. Script/Multiplayer/ParkHanSolLobbyMenuController.cs";
            if (!File.Exists(path))
            {
                errors.Add($"lobby_settings_source_missing path={path}");
                return;
            }
                     ...
                     })
            {
                ...
                {
                    errors.Add(
                        $"lobby_master_ownership_contract marker={marker}");
                }
            }
        }
