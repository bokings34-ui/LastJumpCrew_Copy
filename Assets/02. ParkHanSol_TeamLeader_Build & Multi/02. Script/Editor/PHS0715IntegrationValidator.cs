    ...
    public static class PHS0715IntegrationValidator
    {
        ...
        private static void ValidateRunSessionRootPrefab(ICollection<string> errors)
        {
            ...
            try
            {
                ...
                var gameOverSequences = prefab.GetComponentsInChildren<
                    NetworkGameOverSequenceCoordinator>(true);
                Require(
                    gameOverSequences.Length == 1,
                    $"run_session_root_game_over_sequence_count_invalid " +
                    $"expected=1 actual={gameOverSequences.Length}",
                    errors);
                var gameOverSequence = prefab.GetComponent<
                    NetworkGameOverSequenceCoordinator>();
                Require(
                    gameOverSequence != null,
                    "run_session_root_game_over_sequence_owner_invalid expected=root",
                    errors);
                if (gameOverSequence != null)
                {
                    var serializedGameOverSequence = new SerializedObject(
                        gameOverSequence);
                    Require(
                        gameOverSequence.enabled,
                        "run_session_root_game_over_sequence_disabled",
                        errors);
                    Require(
                        Mathf.Approximately(
                            serializedGameOverSequence
                                .FindProperty("timeOverDurationSeconds")
                                ?.floatValue ?? 0f,
                            8.5f),
                        "run_session_root_game_over_time_over_duration_invalid expected=8.5",
                        errors);
                    Require(
                        Mathf.Approximately(
                            serializedGameOverSequence
                                .FindProperty("shipDestroyedDurationSeconds")
                                ?.floatValue ?? 0f,
                            5f),
                        "run_session_root_game_over_ship_destroyed_duration_invalid expected=5",
                        errors);
                }
