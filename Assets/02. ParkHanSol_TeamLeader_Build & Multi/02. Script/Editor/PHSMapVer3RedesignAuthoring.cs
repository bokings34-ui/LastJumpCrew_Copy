...
namespace LastJumpCrew.ParkHanSol.Editor
{
    public static class PHSMapVer3RedesignAuthoring
    {
        ...
        private static void AuthorGravityLayout(Scene scene)
        {
            ...
            ConfigureInteriorContainmentEnvelope(scene);
        ...
        private static void ConfigureInteriorContainmentEnvelope(Scene scene)
        {
            var containmentRoot = Find(scene, "PHS_Map_Runtime/PHS_InteriorContainment");
            var controller = containmentRoot.GetComponent<NetworkInteriorContainmentController>();
            Require(controller != null, "interior_containment_controller_missing");

            var envelope = FindDirectChild(containmentRoot, "PHS_InteriorSafetyEnvelope");
            if (envelope == null)
            {
                envelope = new GameObject("PHS_InteriorSafetyEnvelope").transform;
                envelope.SetParent(containmentRoot, false);
            }

            envelope.position = new Vector3(0f, 0f, 48f);
            envelope.rotation = Quaternion.identity;
            envelope.gameObject.layer = LayerMask.NameToLayer("NoPlayerInteract");

            var containmentTrigger = envelope.GetComponent<BoxCollider>()
                ?? envelope.gameObject.AddComponent<BoxCollider>();
            containmentTrigger.center = Vector3.zero;
            containmentTrigger.size = new Vector3(82f, 22f, 152f);
            containmentTrigger.isTrigger = true;

            var volume = envelope.GetComponent<NetworkInteriorContainmentVolume>()
                ?? envelope.gameObject.AddComponent<NetworkInteriorContainmentVolume>();
            SetSerializedValue(volume, "controller", controller);
            SetSerializedValue(volume, "containmentTrigger", containmentTrigger);
        }
        ...
        {
            Move(scene, "PHS_Map_Runtime/Interaction/PHS_BatteryVending", new Vector3(-3f, -3.72f, 69f));
