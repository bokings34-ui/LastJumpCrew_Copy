...
{
    public sealed class PHSAnimatedWrenchItemUse : MonoBehaviour, IUsableItem
    {
        ...
        public void Use(IItemHolder holder, IInteractable target)
        {
            ...
            {
                Debug.LogError("PHS_WRENCH_USE_FAILED reason=holder_component_missing", this);
                return;
            }
            ...
            {
                Debug.LogError(
                    $"PHS_WRENCH_USE_FAILED reason=action_contract_missing holder={holderComponent.name}",
                    this);
                return;
            }
